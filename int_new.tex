...
\noindent
To test this, the thesis develops three hypotheses, each corresponding to a different class of perturbation mechanism.
They are not alternatives in the sense of competition, but structurally distinct explanatory possibilities—each capturing a different way in which synthetic presence may modulate evaluative dynamics.

\noindent
The thesis therefore formulates three hypotheses, each corresponding to a distinct
class of perturbation mechanism within the cognitive--affective architecture of
moral judgment. 
They do not compete for the same explanatory domain; rather, each isolates a 
different structural pathway through which synthetic presence may modulate the 
evaluative transformation from perception to action. 
Taken together, they delineate the theoretical space within which the experiment 
operates.
...
\noindent
Together, these hypotheses constitute the inferential scaffold for the experimental design developed later in the thesis. 
They translate the abstract problem of machine-mediated moral cognition into empirically testable pathways, each grounded in the theoretical architecture developed 
throughout the introduction: an architecture in which moral behaviour is understood as 
the product of a field-sensitive cognitive process, vulnerable to deformation by synthetic 
co-presence.
