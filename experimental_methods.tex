...
\noindent
To determine whether specific cognitive–affective regimes exhibit differential sensitivity to robotic presence, we conducted a stratified linear regression analysis within each of the three personality clusters derived from the PCA–$k$-means framework. Donation amount served as the dependent variable, while experimental condition (Control vs.\ Robot) functioned as the primary predictor, allowing us to probe whether the perturbative effect of $\gamma_R$ is uniformly distributed or selectively amplified within particular psychological ecologies.

\noindent
Results reveal a strikingly asymmetric pattern of moral responsiveness. In \textbf{Cluster 1}, robotic presence is associated with a substantial reduction in donation behaviour (\(\beta = -1.33\)), approaching conventional significance (\(p = .091\)), and accounting for a non-trivial proportion of variance (\(R^2 = 0.087\)). This cluster-level attenuation is consistent with earlier descriptive analyses indicating that individuals within this regime tend to exhibit lower empathizing scores and higher systemizing tendencies—traits that may weaken the affective pathways through which Watching Eye stimuli ordinarily exert their normative pull.

\noindent
By contrast, \textbf{Clusters 0 and 2} display negligible perturbation effects: Cluster 0 yields a coefficient near zero (\(\beta \approx 0\), \(p > .70\)), and Cluster 2 exhibits only a minor, non-significant negative trend (\(\beta = -0.28\), \(p > .70\)). This heterogeneity underscores a central theoretical point: \textbf{robotic presence does not impose a global moral influence}, but instead interacts with latent psychological structures in a manner consistent with regime-dependent refractive effects.

\noindent
These findings reinforce the interpretive shift introduced in the previous sections: individual differences should not be treated as additive covariates, but as \textbf{distinct cognitive–affective topologies}—internal landscapes that modulate the stability and amplitude of moral salience transmission under perturbative conditions. In this sense, the regression outcomes illuminate how the Watching Eye stimulus and the robotic presence jointly operate within a structured evaluative manifold. For some individuals, the addition of $\mathscr{R}$ fragments the normative coherence of the environment; for others, the moral field remains sufficiently resilient to sustain ordinary prosocial output.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{new_plots/cluster_regression_coefficients.png}
	\caption{Regression coefficients for the robot condition within each personality cluster, with 95\% confidence intervals. Cluster 1 exhibits a pronounced negative effect, while Clusters 0 and 2 display negligible or non-significant coefficients. This pattern demonstrates differentiated moral responsiveness to robotic presence, conditioned by latent cognitive–affective regimes.}
	\label{fig:cluster-regression}
\end{figure}

\begin{center}
	\begin{tcolorbox}[colback=white,colframe=black!60,
		title=Conclusion: Differentiated Moral Sensitivity to Robotic Presence~\ref{conc:regression_cluster_specific}]
		Robotic presence does not exert a uniform moral influence. Instead, its perturbative effect emerges selectively through the structured configurations of latent psychological regimes. Cluster-specific regression analysis demonstrates that moral attenuation is concentrated within particular cognitive–affective ecologies, confirming that the ethical salience of synthetic agents is not globally encoded but \textbf{contextually realised through trait-dependent evaluative topologies}.
	\end{tcolorbox}
\end{center}

% Keep the label exactly as provided
\label{conc:regression_cluster_specific}


\noindent
This cluster-level analysis therefore advances the broader conceptual arc of the chapter. The perturbative force of $\mathscr{R}$ is neither binary nor homogeneous. It refracts through psychological structures that differ in their sensitivity to moral cues, their susceptibility to ontological ambiguity, and their capacity to integrate artificial co-agents into the evaluative architecture of practical reasoning. What remains is to examine the extent to which these findings persist when the classical linear assumptions are relaxed, and when the inferential dynamics are modelled within more flexible probabilistic and interaction-sensitive frameworks.
