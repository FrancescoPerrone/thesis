...
[sibling distance=6cm]
child { node {Factual} 
    child { node { \\ \textit{$p$} \textbf{is} the case \\ is \textit{$p$} the case?}}} % <- end fact
child { node {? \\ (Moral)} } % <- end any
child { node {Normative} 
    child{ node {\textit{$p$} \textbf{ought} to be the case \\ whether $p$ is the case or not}}} %<-end cognition
; %<-end node
\end{tikzpicture}
